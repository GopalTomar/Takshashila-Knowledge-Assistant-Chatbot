...
Production layout: GitHub Pages (frontend) → Render Free (API + Mattermost) ←
GitHub Actions (KB refresh). Start with `python scripts/smoke_test.py --api <render-url>`.

| Symptom | Likely cause | Fix |
|---|---|---|
| `/api/health` → `"status": "starting"` for minutes | first boot is downloading/decrypting the KB bundle and building BM25 | wait (a few minutes on Render Free at 0.1 CPU); check `kb_sync.last_error` |
| `/askkb` replies "starting up"; web UI "Service starting…" | Render Free slept after ~15 idle min and is re-loading the KB | ask again in ~2 min; optional keep-awake: GitHub variable `RENDER_KEEPALIVE_URL` |
| Render deploy: "Out of memory (used over 512Mi)" | `KB_LOW_MEMORY` not `true`, or a non-ONNX image | keep `KB_LOW_MEMORY=true` (render.yaml) and the repository Dockerfile (`EMBEDDING_BACKEND=onnx`) |
| `/api/health` → `"degraded"`, `error: FileNotFoundError … faiss.index` | no KB locally and bundle sync not configured | set `KB_BUNDLE_MANIFEST_URL` + `KB_BUNDLE_KEY`, (Render: see DEPLOYMENT.md §3) |
...
| Browser: CORS error | frontend origin not allowed | `CORS_ALLOW_ORIGINS=https://<user>.github.io` on Render (origin only, no path) |
| Web UI: "Your access token was not accepted" | token not in `API_ACCESS_TOKENS` | fix the Render environment variable or remove the token in the UI |
...
| Buttons do nothing | `MATTERMOST_BOT_PUBLIC_URL` unset or Mattermost can't reach Render | set it to the Render HTTPS URL (`https://<service>.onrender.com`); allow outgoing connections in Mattermost |
